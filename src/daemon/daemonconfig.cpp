...
DaemonConfig::DaemonConfig(std::string config_file, std::string config_defaults_file) {
    ...
    socket = getBoolean(&ini, &defaultIni, "daemon", "socket", false);
    serial_port = getBoolean(&ini, &defaultIni, "daemon", "serial_port", false);
    max_tableau_size = getInt(&ini, &defaultIni, "daemon", "max_tableau_size", 1);
}
